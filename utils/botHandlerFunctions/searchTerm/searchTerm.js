...
export const searchItem = async (messageData) => {
    const { phoneNumber, text, btnReply, listReply, lastMessage, image = {}, location, messagingProduct, profileName, user, vendor, s_u_ln, lang, s_v_ln } = messageData;
        ...
        try {
            ...
            if (matchVendors.length > 0) {
                ...
                for (const { vendor, pendingQueries } of allPendingQueries) {
                    ...
                    for (const query of pendingQueries) {
                        const button = [
                            { id: `Yes_avl|${query.queryId}`, title: lang[s_u_ln].YES },
                            { id: "No_avl", title: lang[s_u_ln].NO }
                        ];

                        await sendButtonMessage(vendor?.phoneNumber, `${lang[s_u_ln].USER_SEARCHING} ${query.product}. ${lang[s_u_ln].AVAILABILITY_QUESTION}`, button, `0.1.7_${query.queryId}`);
                    }
                }
            }
        } catch (error) {
    ...
    // }
        ...
        if (userFound.coins >= 1) {
            ...
            await sendTextMessage(userFound.phoneNumber, `${lang[s_u_ln].PRICE}: ${priceProd}`);
        } else {
            return await sendTextMessage(userFound.phoneNumber, lang[s_u_ln].INSUFFICIENT_COINS, "error");
        }
    }
