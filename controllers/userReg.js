...
export const userSignUp = async (req,res) =>{
  ...
  try {
      // Check if email OR phone number already exists in any registration source
      const existingUsers = await User.find({
          $or: [{ email }, { phoneNumber }]
      });

      if (existingUsers.length > 0) {
          return res.status(400).json({ message: "User already registered with another source" });
      }
